...
function geoSetup() {
  ...
  if (!navigator.geolocation) {
    alert(
      "Your device does not support geolocation! This page will not work properly!"
    );
  } else {
...
function geocodeAddress(address, option, i) {
    ...
    if (status == "OK") {
      ...
      if (option == 0) {
        ...
        var latDiff = Math.abs(
          Math.abs(latitude) - Math.abs(results[0].geometry.location.lat())
        );
        var lngDiff = Math.abs(
          Math.abs(longitude) - Math.abs(results[0].geometry.location.lng())
        );
        ...
        if (userMarker == null) {
          ...
          //use same marker and change marker position
          userLocation = results[0].geometry.location;
          latitude = results[0].geometry.location.lat();
          longitude = results[0].geometry.location.lng();
          converUserLocationToReadableAddressSetup();
